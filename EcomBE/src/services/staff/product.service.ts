...
class StaffProductService {
   ...
   */
  async reviewProductApproval(
    staffId: string,
    input: ReviewProductApprovalInput
  ) {
    ...
    if (!allowedStatuses.includes(status as any)) {
      throw new ValidationError(
        `Status must be ${ProductStatus.ACTIVE} or ${ProductStatus.REJECTED}`
      );
    }
  ...
  async banProduct(staffId: string, productId: string, reason: string) {
    ...
    await auditLogRepository.create({
      userId: staffId,
      action: AuditAction.BAN_PRODUCT,
      resource: AuditResource.PRODUCT,
      resourceId: productId,
      details: JSON.stringify({ reason }),
    });
